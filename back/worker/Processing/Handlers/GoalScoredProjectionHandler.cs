...
public sealed class GoalScoredProjectionHandler : IEventProjectionHandler
{
    private static readonly string[] SupportedEventTypes = new[] { "GoalScoredEvent", "goal.scored" };

    private readonly IProjectionQueryRepository _projectionQueries;
    private readonly IReadModelStore _readModelStore;
    private readonly ILogger<GoalScoredProjectionHandler> _logger;

    public GoalScoredProjectionHandler(
        IProjectionQueryRepository projectionQueries,
        IReadModelStore readModelStore,
        ILogger<GoalScoredProjectionHandler> logger)
    {
    ...
    public async Task HandleAsync(InboxEvent inboxEvent, CancellationToken ct)
    {
        ...
        var strikersJson = await _projectionQueries.BuildStrikersJsonAsync(championshipId.Value, ct);
        if (!string.IsNullOrWhiteSpace(strikersJson))
        {
            await _readModelStore.SaveStrikersAsync(championshipId.Value, strikersJson, ct);
        }
    }
